...
db.getCollection('reference_Meta').remove({"type": "property","name": "date_picker_format"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "month_picker_format"});//
...
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_connect_host"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_mdm_host"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_auth_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_auth_validate_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_connect_ui_host"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "platform_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "nifi_host"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "nifi_port"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "nifi_endPointVsNifiPort"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_logout_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_invalidate_session_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "spring_redis_port"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "spring_redis_host"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "spring_profiles_active"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "delivery_items_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "gmr_create_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_connect_mongo_uri"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "mongo_container_name"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "mongo_port"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "mongo_database"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "mongo_username"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "mongo_password"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "spring_data_mongodb_uri"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_UserAuthentication_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "eka_validateToken_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "contract_details_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "assay_url"});//
db.getCollection('reference_Meta').remove({"type": "property","name": "stock_url"});//
db.getCollection('reference_Meta').remove({"type": "processor","name": "editgmrbasic"});//
   ...
   "name" : "User_Info",
    ...
    "fields" : {
        ...
        "website" : {
         ...
         "housenoStreetname" : {
            "housenoStreetname" : "House no., Street name",
            "labelKey" : "housenoStreetname",
            "type" : "textbox",
            "dataType" : "String"
        },
          "company" : {
...
db.getCollection('reference_Meta').insert({
    ...
    "flow" : {
        ...
        "assayinglisting" : {
            ...
            "objectAction" : "UPDATE",
            "getDataFromExternalAPI" : true,
			"url" : "http://192.168.1.225:3333/collection/assayList",
            "layout" : {
                "name" : "assay",
                "selection" : "row",
                "header" : {
                    ...
                    "outcomes" : [ 
                        {
                            ...
                            "result" : {
                                "success" : {
                                    "message" : "Profile got replaced successfully"
                                },
                                "failure" : {
                                    "message" : "Please fix errors "
                                },
                                "successWithWarning" : {
                                    "message" : "Profile got replaced with some errors"
                                }
                            }
                        }, 
                        {
        ...
        "editpackaging" : {
            ...
            "objectAction" : "UPDATE",
            "url" : {
                "packagingApi" : "http://192.168.1.225:3333/collection/assayAndStockList"
            },
            "layout" : {
                "name" : "pack",
                "type" : "row",
...
"name" : "editgmrbasic",
    ...
    "refType" : "app",
    "refTypeId" : "467a28cc-bc93-4e38-8ff5-0a56ae128f3b",
    "snippet" : "var intGmrRefNo = req.body.selectedData.selectedData['Internal GMR Ref No'];\r\n  var collection = {\r\n    method: 'POST',\r\n    url: 'http://192.168.1.169:8001/api/logistic/viewGmr',\r\n    headers: {\r\n      'Content-Type': 'application/json',\r\n      Accept: 'application/json',\r\n      username: 'e-bolprpa'\r\n    },\r\n    body: {\r\n      intGmrRefNo: intGmrRefNo\r\n    },\r\n    json: true\r\n  };\r\n  function get_collection() {\r\n    return new Promise(function(resolve, reject) {\r\n      console.log('Request - ' + collection.url);\r\n      request(collection, function(err, response, body) {\r\n        console.log('Response - ' + body);\r\n        if (err || body.error) reject(body);\r\n        else {\r\n          console.log('Success');\r\n          resolve(body);\r\n        }\r\n      });\r\n    });\r\n  }\r\n\r\n  get_collection()\r\n    .then(result => {\r\n      var data = {};\r\n      data.CountryOfOrigin = result.countryOfOriginCountryName;\r\n      data.ExpectedArrivalDate = result.arrivalDate;\r\n      data.LoadingDate = result.loadingDate;\r\n      data.LoadingLocationCity = result.loadingCityName;\r\n      data.LoadingLocationCountry = result.loadingCountryName;\r\n      data.SupplierRepresentative = result.supplierRepresentaive;\r\n      data.cpAddress = result.supplierAddress;\r\n      data.cpName = result.supplier;\r\n      data.deliveryItemRefNo = result.deliveryItemRefNo;\r\n      data.qualityName = result.qualityName;\r\n      data.dischargeCityId = result.dischargeCityName;\r\n      data.dischargeCountryId = result.dischargeCountryName;\r\n\r\n      res.status(200).send(data);\r\n    })\r\n    .catch(err => {\r\n      res.status(500).send(err);\r\n    });",
    "sys__UUID" : "ec83540d-2bcb-44bc-9b78-cfaf5ea2a0c8",
    "sys__createdOn" : ISODate("2019-06-17T08:15:03.550Z"),
